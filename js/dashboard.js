...
function querboxEditableOrNot(userType) {
    ...
    // if(userType=='VIEWER'){
    //     document.getElementById('queryInputBox').disabled=true;
    //     document.getElementById('queryInputBox').setAttribute('title','You Can Not Make Chakges In Table.');
    // }
}
...
function loadTableName(databaseName, masterTableName) {
    ...
    for (var i = 0; i < masterTableName.length; i++) {
        ...
        if (hostName != 'localhost') {
            tempDatabaseName = masterTableName[i].table_schema;
            tempTableName = masterTableName[i].table_name;
        } else {
            tempDatabaseName = masterTableName[i].TABLE_SCHEMA;
            tempTableName = masterTableName[i].TABLE_NAME;
        }
...
function loadColoumneName(tableId, columnNameArray) {
    ...
    else if (listInstance.innerHTML == '' && hostName != 'localhost') {
...
function showTableDataInview(selectIconInstance, callType, selectQuery) {
    ...
    if (callType == 'DEFAULT') {
        ...
        enableHidden("playButtonIcon", true);
        enableHidden("playSpinnerIcon", false);
        enableHidden('showTableDataDiv', true);
        hideLoadingMessage('loadingTableMessage', "Loading Data.Please Wait...", false);
    }
                ...
                if (queryArray[i].trim().length > 0) {
                    $.post('allQuery.php', {
                        ...
                        if(data=='VIEWER ERROR'){
                            alert('you can not make changes in database.');
                            enableHidden("playButtonIcon", false);
                            enableHidden("playSpinnerIcon", true);
                            hideLoadingMessage('loadingTableMessage', "Loading Data. Please Wait...", true);
                            flagForContinousRun=false;
                            return;
                        }
