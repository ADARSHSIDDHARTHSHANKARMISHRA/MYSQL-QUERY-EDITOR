...
switch ($currentPage) {
  ...
  case 'createDatabaseAccount.php':
    $keywords = 'MYSQL QUERY EDITOR';
    $title = 'Create Account - ' . $clientName;
    $description = $desc;
    break;
  ...
  case 'allMember.php':
    $keywords = 'MYSQL QUERY EDITOR';
    $title = 'Manage Member - ' . $clientName;
    $description = $desc;
    break;

  case 'myProfile.php':
    $keywords = 'MYSQL QUERY EDITOR';
    $title = 'My Account - ' . $clientName;
    $description = $desc;
    break;

  case 'myMessage.php':
    $keywords = 'MYSQL QUERY EDITOR';
    $title = 'All Message - ' . $clientName;
    $description = $desc;
    break;

  case 'queryLog.php':
    $keywords = 'MYSQL QUERY EDITOR';
    $title = 'Log Records - ' . $clientName;
    $description = $desc;
    break;
...
function logoutDetetection($pageName)
{
  ...
  if (isset($_SESSION['login']) && isset($_SESSION['LAST_ACTIVITY'])) {
    ...
    // echo "User is log in".$_SESSION['LAST_ACTIVITY']." ".time()." ".(time() - $_SESSION['LAST_ACTIVITY']);
    // echo time() - $_SESSION['LAST_ACTIVITY'] > 120;
  }
}
?>
